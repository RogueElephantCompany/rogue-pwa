...
class AllRepairs extends Component {
  ...
  render() {
    ...
    const filteredRepairs = repairs.slice().reverse()
      .filter(repair => Number(repair.date.slice(-4)) === Number(this.state.year))
      .filter(prev => {
        return (prev.user.email.toLowerCase().match(inputVaue) ||
            ...
            <div>
              ...
              <div style={{ textAlign: 'center', marginTop: '20px' }}>
                <button
                  id="signin-button"
                  type='submit'
                  onClick={this.hideDetails}>Hide </button>
              </div>
            </div>
          ) : ''
        }
